...
def chain_converged(
    ...
    verbose: bool = True,
    detail_prefix_lines: list[str] | None = None,
) -> bool:
    import mepd.chainhelpers as ch
    fraction_freeze = getattr(chain_new.parameters, "fraction_freeze", 0.1)
    ...
    if ind_ts_guess == 0 or ind_ts_guess == len(chain_new)-1:
        if verbose and _rich_available:
            _console.print(
                "[yellow]⚠ Warning: TS guess is at the end of the chain. This might indicate a problem with the initial guess.[/yellow]")
        elif verbose:
    ...
    criteria_values = [
        max(rms_gperps),
        sum(rms_gperps) / len(chain_new),
        ts_guess_grad,
        ts_triplet_gspring,
        max(springgrads),
        float(np.abs(chain_new.get_eA_chain() - chain_prev.get_eA_chain())),
    ]
    criteria_thresholds = [
        parameters.max_rms_grad_thre,
        parameters.rms_grad_thre,
        parameters.ts_grad_thre,
        parameters.ts_spring_thre,
        parameters.ts_spring_thre,
        parameters.barrier_thre,
    ]

    detail_lines = []
    for line in detail_prefix_lines or []:
        text = str(line).strip()
        if text:
            detail_lines.append(text)
    for name, converged_flag, value, thresh in zip(
        CRITERIA_NAMES, criteria_converged, criteria_values, criteria_thresholds
    ):
        status = "OK" if converged_flag else "NO"
        detail_lines.append(
            f"{status} {name}: {float(value):.4g} <= {float(thresh):.4g}"
        )
    set_monitor_details(detail_lines)
    ...
    if not verbose:
        ...
        table = Table(
            title="[bold]Convergence Criteria[/bold]", box=box.ROUNDED)
