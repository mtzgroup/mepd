...
class ProgressPrinter:
    ...
    def set_monitor_details(self, detail_lines: list[str] | None) -> None:
        """Set persistent detail lines for the active monitor in live output."""
        with self._lock:
            ...
            if normalized == list(state.get("detail_lines") or []):
                return
            state["detail_lines"] = normalized
    ...
    def _build_live_rows(self) -> tuple[list[tuple[str, str]], dict]:
        ...
        if main_ready:
            ...
            main_detail_lines = list(main_state.get("detail_lines") or [])
            if main_detail_lines:
                chain_ascii = f"{chain_ascii}\n" + "\n".join(main_detail_lines)
...
def _endpoint_energy_kcal_delta(start_node, end_node) -> float | None:
    def _energy(node):
        try:
            return float(node.energy)
        except Exception:
            try:
                return float(getattr(node, "_cached_energy"))
            except Exception:
                return None

    e_start = _energy(start_node)
    e_end = _energy(end_node)
    if e_start is None or e_end is None:
        return None
    return abs(e_start - e_end) * 627.5


def _endpoint_rmsd(start_node, end_node) -> float | None:
    try:
        start_coords = np.asarray(start_node.coords, dtype=float)
        end_coords = np.asarray(end_node.coords, dtype=float)
    except Exception:
        return None
    if start_coords.shape != end_coords.shape or start_coords.size == 0:
        return None

    inds_start = getattr(start_node, "comparison_atom_indices", None)
    inds_end = getattr(end_node, "comparison_atom_indices", None)
    if inds_start is not None and inds_end is not None:
        try:
            arr_start = np.asarray(inds_start, dtype=int)
            arr_end = np.asarray(inds_end, dtype=int)
            if (
                arr_start.size > 0
                and arr_start.shape == arr_end.shape
                and np.array_equal(arr_start, arr_end)
                and int(np.min(arr_start)) >= 0
                and int(np.max(arr_start)) < len(start_coords)
                and int(np.max(arr_end)) < len(end_coords)
            ):
                start_coords = start_coords[arr_start]
                end_coords = end_coords[arr_end]
        except Exception:
            pass

    try:
        rmsd, _ = align_geom(refgeom=end_coords, geom=start_coords)
        return float(rmsd)
    except Exception:
        try:
            return float(np.sqrt(np.mean(np.sum((start_coords - end_coords) ** 2, axis=-1))))
        except Exception:
            return None
...
def _endpoint_difference_summary(
    ...
    *,
    ...
    if graph_start is not None and graph_end is not None:
        try:
            same_connectivity = graph_start.remove_Hs().is_bond_isomorphic_to(
                graph_end.remove_Hs()
            )
    ...
    return (
        ...
        f"deltaE={_format_optional_float(delta_e)} kcal/mol, "
        f"rmsd={_format_optional_float(rmsd)}"
    )
...
def ascii_profile_for_chain(chain, width: int = 60, height: int = 12) -> str:
    try:
        ...
        if energies and np.isfinite(energies[0]):
            energies = (np.asarray(energies, dtype=float) - float(energies[0])) * 627.5
    ...
    endpoint_meta = f"endpoints differ in: {_truncate_meta(endpoint_lines[0], 70)}\n"
    for line in endpoint_lines[1:]:
        endpoint_meta += f"                      {_truncate_meta(line, 70)}\n"
    meta = (
...
def set_monitor_details(detail_lines: list[str] | None):
    """Set persistent detail lines for the active monitor row."""
    printer = get_progress_printer()
    printer.set_monitor_details(detail_lines)
