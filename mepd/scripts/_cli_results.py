...
def _cost_report_path(output_dir: Path, base_name: str) -> Path:
    return Path(output_dir) / f"{base_name}_cost.json"
...
def _write_cost_report(
    ...
    *,
    ...
    gradient_calls_geometry_optimizations: int | None = None,
    ...
    payload: dict[str, object] = {
        ...
        "elapsed_minutes": float(max(0.0, elapsed_seconds) / 60.0),
    }
    if mode is not None:
        payload["mode"] = str(mode)
    if gradient_calls_total is not None:
        payload["gradient_calls_total"] = int(gradient_calls_total)
    if gradient_calls_geometry_optimizations is not None:
        payload["gradient_calls_geometry_optimizations"] = int(
            gradient_calls_geometry_optimizations
        )
    if metadata:
        payload["metadata"] = metadata
    _write_json_atomic(cost_fp, payload)
...
def _write_run_status(
    ...
    *,
    ...
    network_completion: bool | None = None,
    path_min_method: str | None = None,
    output_chain_path: Path | None = None,
    tree_path: Path | None = None,
    network_completion_dir: Path | None = None,
    ...
    if network_completion is not None:
        payload["network_completion"] = network_completion
    ...
    if network_completion_dir is not None:
        payload["network_completion_dir"] = str(network_completion_dir)
...
def _resolve_status_artifact(path_text: str) -> tuple[Path, str]:
    ...
    if src.is_file():
        ...
        if src.suffix == ".xyz":
            ...
            candidates.append((src.parent / f"{stem}_network_completion" / f"{stem}_request_manifest.json", "request_manifest"))
    elif src.is_dir():
        ...
        if src.name.endswith("_network_completion"):
            stem = src.name[: -len("_network_completion")]
            ...
            candidates.append((src.parent / f"{src.name}_network_completion" / f"{src.name}_request_manifest.json", "request_manifest"))
    else:
