...
class GeodesicOptimizer:
    ...
    def __init__(
        ...
        batch_evaluator: Callable[[torch.Tensor, torch.device, torch.dtype], Tuple[torch.Tensor, torch.Tensor]] | None = None,
    ):
        """
        ...
        if self._batch_evaluator is None:
            ...
            self.calc, self.raw_module = None, None
            log.info("Loaded engine-backed MLPGI evaluator.")
            self._emit_status(f"Loaded engine-backed MLPGI evaluator on {self.device.type}")
    ...
    def _evaluate_batch(self, nodes_for_eval: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        if self._batch_evaluator is not None:
            return self._batch_evaluator(nodes_for_eval, self.device, self.dtype)
        return evaluate_mlp_batch(
            nodes_for_eval,
            self.calc,
            self.raw_module,
            self._template_atoms,
            self.Z,
            self.device,
            self.dtype,
            self.backend,
    ...
    def _evaluate_path_energies_forces(
        ...
        E_main, F_main_flat = self._evaluate_batch(path_nodes_tensor)
        ...
        if evaluate_midpoints and path_nodes_tensor.size(0) > 1:
            ...
            E_mid_batch, F_mid_flat_batch = self._evaluate_batch(geom_midpoints_coords)
            E_mid = E_mid_batch
