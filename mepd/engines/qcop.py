...
class QCOPEngine(Engine):
    ...
    def _geometry_optimizer_keywords(
        ...
        kwds = dict(DEFAULT_GEOMETRY_OPTIMIZER_KWDS)
        kwds.update(dict(self.geometry_optimizer_kwds or {}))
    ...
    def _compute_geom_opt_result(self, node: StructureNode, keywords=None):
        ...
        if "terachem" not in self.program:
            ...
            tc_keywords = dict(
                getattr(self.program_args, "keywords", {}) or {})
            tc_keywords.update(
                {
                    "purify": "no",
                    "new_minimizer": "yes",
        ...
        def _hessian_payload_is_usable(output_obj: Any) -> bool:
            ...
            if isinstance(trajectory, list):
                for entry in trajectory:
                    ...
                    entry_modes = getattr(
                        entry_results, "normal_modes_cartesian", None)
                    ...
                    if _has_square_hessian(getattr(entry_results, "hessian", None)):
                        return True
                    entry_files = _coerce_files_mapping(
                        getattr(entry_results, "files", None))
    ...
    def compute_geometry_optimization(self, node: StructureNode, keywords=None) -> list[StructureNode]:
        """
    ...
    def _structure_nodes_from_structures(self, structures: list[Structure]) -> list[StructureNode]:
        nodes: list[StructureNode] = []
        graphs_disabled = bool(
            getattr(self, "disable_molecular_graphs", False))
        for structure in structures:
            ...
            node = StructureNode(
                ...
                graph=graph,
                has_molecular_graph=(
                    graph is not None and not graphs_disabled),
            )
            nodes.append(node)
        return nodes
