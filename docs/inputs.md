...
It is derived from `mepd/inputs.py` on this branch, so it reflects the
...
| `ase_engine_kwds` | `table` | `{}` | ASE-only options such as `geometry_optimizer` and `transition_state_optimizer` |
...
- If omitted, fallback defaults are `model_path = "/home/diptarka/fairchem/esen_sm_conserving_all.pt"`
  and `device = "cuda"`.
- `optimizer_kwds` configures the path minimizer optimizer (CG/VPO/LBFGS/etc.), not
  the ASE geometry optimizer class. Use `ase_engine_kwds.geometry_optimizer` for ASE.
...
When `disable_molecular_graphs = true`, graph-based identity checks are disabled.
If `skip_identical_graphs = true` is also set, `RunInputs` emits a warning because
`skip_identical_graphs` is ignored without molecular graphs.
...
| `early_stop_stage` | `false` | Two-stage DL-Find: run loose minimization, do elem-step check, then continue with tight settings only when elementary |
| `early_stop_loose_overrides` | `{}` | Parameter overrides for the loose stage (same keys as `path_min_inputs`, including nested `dlfind_keywords`) |
...
Use this section with `engine_name = "qcop"` or `"chemcloud"` to pass keywords
to the geomeTRIC geometry optimizer.
...
Use this section with `engine_name = "ase"`.

| Key | Type | Default | Notes |
| --- | --- | --- | --- |
| `geometry_optimizer` | `str` | `"LBFGSLineSearch"` | One of `LBFGS`, `BFGS`, `FIRE`, `LBFGSLineSearch`, `MDMin` |
| `transition_state_optimizer` | `str` | `"SELLA"` | TS optimizer backend for ASE TS optimization |
