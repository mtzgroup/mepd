...
This tutorial walks through using MEPD for finding reaction pathways.

## Prerequisites

Install MEPD and dependencies:
...
```python
from qcio import Structure
from mepd import MSMEP, RunInputs, StructureNode, ChainInputs
from mepd.engines.qcop import QCOPEngine
import mepd.chainhelpers as ch
...
# Load chain from xyz
from mepd import Chain, ChainInputs
...
### Problem: MSMEP keeps splitting into too many conformers

**Solutions:**
- Increase `node_rms_thre` and `node_ene_thre` in `ChainInputs` so small conformer differences are merged more aggressively

### Problem: MSMEP splits spuriously into high-energy molecules

**Solutions:**
- Use less aggressive optimizer settings, such as a smaller `timestep` or smaller norm scaling
