...
import mepd.NetworkBuilder as network_builder_module
...
def test_graph_equivalent_requires_matching_stereochemical_smiles(monkeypatch):
    builder = NetworkBuilder(
        data_dir=Path("."),
        chain_inputs=ChainInputs(),
        network_inputs=NetworkInputs(verbose=False),
    ...
    left = _node(0.74, 0.0)
    right = _node(0.74, 0.0)
    assert builder._graph_equivalent(left, right) is True

    def _fake_stereo_key(node):
        if node is left:
            return "C[C@H](F)Cl"
        if node is right:
            return "C[C@@H](F)Cl"
...
def test_create_rxn_network_from_paths_raises_when_no_valid_structures(monkeypatch):
    builder = NetworkBuilder(
        data_dir=Path("."),
        chain_inputs=ChainInputs(),
        network_inputs=NetworkInputs(verbose=False),
    ...
    monkeypatch.setattr(builder, "_load_network_data", lambda msmep_paths: ([], {}))

    with pytest.raises(ValueError, match="No valid elementary-step leaves"):
        builder.create_rxn_network_from_paths([Path(".")])
