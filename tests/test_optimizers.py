...
def _image_rms_steps(step: np.ndarray) -> np.ndarray:
    flat = step.reshape(step.shape[0], -1)
    return np.linalg.norm(flat, axis=1) / np.sqrt(flat.shape[1])


def _assert_max_image_rms_step(step: np.ndarray, max_step_norm: float) -> None:
    assert np.max(_image_rms_steps(step)) <= max_step_norm + 1e-12
...
def test_conjugate_gradient_adapts_timestep_from_gradient_correlation():
    opt = ConjugateGradient(
        ...
        positive_steps_thre=2,
    )

    assert opt.update_timestep_from_correlation(0.2) is None
    msg = opt.update_timestep_from_correlation(0.2)
    assert msg == "dt down: 0.5 -> 0.2"
    assert opt.timestep == pytest.approx(0.2)

    assert opt.update_timestep_from_correlation(0.99) is None
    assert opt.update_timestep_from_correlation(0.99) is None
    msg = opt.update_timestep_from_correlation(0.99)
    assert msg == "dt up: 0.2 -> 0.3"
...
def test_conjugate_gradient_auto_timestep_can_be_disabled():
    ...
    assert msg is None
    assert opt.timestep == pytest.approx(0.5)


def test_conjugate_gradient_scales_step_norm_per_image():
    chain = _make_xy_chain(np.zeros((2, 2, 3)))
    grads = np.array(
        [
            [[3.0, 4.0, 0.0], [0.0, 0.0, 10.0]],
            [[0.06, 0.08, 0.0], [0.0, 0.0, 0.1]],
        ]
    )
    ...
    _assert_max_image_rms_step(steps, 0.2)
    np.testing.assert_allclose(_image_rms_steps(steps)[0], 0.2, atol=1e-12)
    np.testing.assert_allclose(steps[0] / np.linalg.norm(steps[0]), -grads[0] / np.linalg.norm(grads[0]), atol=1e-12)
    np.testing.assert_allclose(_image_rms_steps(steps)[1], np.linalg.norm(grads[1].reshape(-1)) / np.sqrt(grads[1].size), atol=1e-12)
...
def test_amg_scales_step_norm_per_image():
    chain = _make_xy_chain(np.zeros((2, 2, 3)))
    grads = np.array(
        [
            [[100.0, -100.0, 0.0], [0.0, 0.0, 80.0]],
            [[80.0, -90.0, 0.0], [0.0, 60.0, 80.0]],
        ]
    )
    ...
    _assert_max_image_rms_step(step, 0.05)
    np.testing.assert_allclose(
        _image_rms_steps(step),
        [0.05, 0.05],
        atol=1e-12,
    )
...
def test_sgd_respects_converged_nodes():
    chain = _make_xy_chain(
        np.array([[1.0, 0.0], [2.0, -1.0], [0.5, 0.5]]),
        converged=[False, True, False],
    )
    grads = np.array([[1.0, 0.0], [4.0, -3.0], [0.5, 0.5]])
    opt = SGDOptimizer(timestep=0.25, momentum=0.9)
...
def test_sgd_scales_step_norm_per_image():
    chain = _make_xy_chain(np.zeros((2, 2, 3)))
    grads = np.array(
        [
            [[3.0, 4.0, 0.0], [0.0, 0.0, 10.0]],
    ...
    opt = SGDOptimizer(timestep=1.0, max_step_norm=0.2)
...
def test_deterministic_gd_respects_converged_nodes():
    chain = _make_xy_chain(
        np.array([[1.0, 0.0], [2.0, -1.0], [0.5, 0.5]]),
        converged=[False, True, False],
    )
    grads = np.array([[1.0, 0.0], [4.0, -3.0], [0.5, 0.5]])
    opt = DeterministicGradientDescentOptimizer(timestep=0.25)
...
def test_deterministic_gd_scales_step_norm_per_image():
    chain = _make_xy_chain(np.zeros((2, 2, 3)))
    grads = np.array(
        [
            [[3.0, 4.0, 0.0], [0.0, 0.0, 10.0]],
    ...
    opt = DeterministicGradientDescentOptimizer(timestep=1.0, max_step_norm=0.2)
...
def test_fire_scales_step_norm_per_image():
    chain = _make_xy_chain(np.zeros((1, 2, 3)))
    grads = np.array([[[3.0, 4.0, 0.0], [0.0, 0.0, 10.0]]])
    opt = FIREOptimizer(timestep=1.0, max_step_norm=0.2)
...
def test_deterministic_gd_auto_timestep_increases_on_plateau():
    ...
    opt = DeterministicGradientDescentOptimizer(
        ...
        min_timestep=0.01,
        max_timestep=1.0,
        corr_increase_thre=0.5,
        corr_decrease_thre=-0.5,
        plateau_window=1,
        plateau_rtol=0.20,
        plateau_growth_lag=1,
    )
...
def test_deterministic_gd_auto_timestep_decreases_on_oscillation():
    ...
    opt = DeterministicGradientDescentOptimizer(
        ...
        min_timestep=0.01,
        max_timestep=1.0,
        corr_increase_thre=0.8,
        corr_decrease_thre=-0.1,
        plateau_window=1,
        plateau_rtol=0.0,
        plateau_growth_lag=1,
    )

    grads1 = np.array([[1.0, 0.0], [0.0, 1.0]])
    chain = opt.optimize_step(chain=chain, chain_gradients=grads1)
    t1 = opt.timestep
    grads2 = -grads1
    chain = opt.optimize_step(chain=chain, chain_gradients=grads2)
...
def test_optimizer_steps_reduce_quadratic_objective():
    ...
    optimizers = [
        ...
        SGDOptimizer(timestep=0.05, momentum=0.8),
        DeterministicGradientDescentOptimizer(timestep=0.05),
