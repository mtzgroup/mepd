...
def _displaced_single_atom_node() -> StructureNode:
    structure = Structure(
        symbols=["H"],
        geometry=np.array([[0.25, -0.10, 0.05]], dtype=float),
        charge=0,
        multiplicity=1,
    )
    return StructureNode(structure=structure)
...
def test_ase_transition_state_returns_final_cached_node(monkeypatch):
    calc = _CountingHarmonicCalculator(k=2.0)
    eng = ASEEngine(calculator=calc)
    ...
    class _FakeSella:
        ...
        def run(self, fmax=0.05, steps=100000000):
            ...
            _ = self.atoms.get_forces()
            from ase.io import Trajectory
            with Trajectory(self.trajectory, "w", self.atoms) as tr:
                tr.write(self.atoms)

    monkeypatch.setattr("mepd.engines.ase.SellaOptimizer", _FakeSella)

    ts_node = eng.compute_transition_state(
        node=_displaced_single_atom_node(),
        keywords={"maxiter": 50, "fmax": 1e-4},
    )

    assert isinstance(ts_node, StructureNode)
    assert ts_node._cached_energy is not None
    assert ts_node._cached_gradient is not None
    assert float(ts_node.energy) >= 0.0


def test_ase_transition_state_raises_when_sella_unavailable(monkeypatch):
    calc = _CountingHarmonicCalculator(k=2.0)
    eng = ASEEngine(calculator=calc)
    monkeypatch.setattr("mepd.engines.ase.SellaOptimizer", None)

    with pytest.raises(ElectronicStructureError, match="sella"):
        eng.compute_transition_state(node=_displaced_single_atom_node())
