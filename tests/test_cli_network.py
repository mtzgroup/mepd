...
from mepd.cli import irc_network as cli_irc_network, network_build as cli_network_build
...
def _call_network_build(**overrides):
    kwargs = dict(tree=None, output=None)
    kwargs.update(overrides)
    return cli_network_build(**kwargs)


def _call_irc_network(**overrides):
    kwargs = dict(
        directory=None,
        pattern="*.xyz",
        recursive=False,
...
def test_network_build_writes_network_from_a_real_tree_directory(tmp_path):
    tree_dir = tmp_path / "tree"
    _write_single_leaf_tree(tree_dir)

    output = tmp_path / "network.json"
    _call_network_build(tree=[tree_dir], output=output)
...
def test_network_build_reports_failure_without_crashing(tmp_path):
    empty_dir = tmp_path / "empty_tree"
    empty_dir.mkdir()
    (empty_dir / "adj_matrix.txt").write_text("1.0\n")
    with pytest.raises(typer.Exit):
        _call_network_build(tree=[empty_dir], output=tmp_path / "out.json")


def test_irc_network_cli_writes_network(tmp_path):
    ...
    output = tmp_path / "network.json"
    _call_irc_network(directory=tmp_path, output=output)
...
def test_irc_network_cli_reports_failure_without_crashing(tmp_path):
    empty_dir = tmp_path / "empty"
    empty_dir.mkdir()
    with pytest.raises(typer.Exit):
        _call_irc_network(directory=empty_dir, output=tmp_path / "out.json")
